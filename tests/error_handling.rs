    ...
    for (invalid_msg, description) in invalid_messages {
        ...
        match result {
            Ok(msg) => {
                ...
                println!("Unexpectedly parsed '{}' ({}): {}", invalid_msg, description, serialized);
            },
            Err(parse_err) => {
                // Verify error provides useful information
                let error_msg = format!("{}", parse_err);
                assert!(!error_msg.is_empty(), "Error should provide description for {}", description);
...
fn test_error_source_chaining() {
    ...
    match result {
        Err(parse_err) => {
            ...
            println!("Error chain length: {}, Full error: {}", chain_length, parse_err);
        },
        Ok(msg) => {
...
fn test_partial_message_errors() {
    ...
    for (partial_msg, description) in partial_messages {
        ...
        match result {
            ...
            Err(parse_err) => {
                let error_msg = format!("{}", parse_err);
                assert!(!error_msg.is_empty(), "Should provide error description for {}", description);
                println!("Parse error for '{}' ({}): {}", partial_msg, description, error_msg);
            }
        }
    }
}
    ...
    for (malformed_msg, description) in malformed_tag_messages {
        ...
        match result {
            Ok(msg) => {
                ...
                println!("Parsed malformed tags '{}' ({}): {}", malformed_msg, description, msg);
            ...
            Err(parse_err) => {
                let error_msg = format!("{}", parse_err);
                println!("Tag parse error '{}' ({}): {}", malformed_msg, description, error_msg);
    ...
    for (malformed_msg, description) in malformed_prefix_messages {
        ...
        match result {
            Ok(msg) => {
                ...
                println!("Parsed malformed prefix '{}' ({}): {}", malformed_msg, description, msg);
            ...
            Err(parse_err) => {
                let error_msg = format!("{}", parse_err);
                println!("Prefix parse error '{}' ({}): {}", malformed_msg, description, error_msg);
                assert!(!error_msg.is_empty(), "Should provide error description");
            }
        }
    }
...
fn test_numeric_response_error_handling() {
    let malformed_numeric_messages = vec![
        ...
        (":server 001 :missing nickname", "missing nickname parameter"),
    ];
    ...
    for (malformed_msg, description) in malformed_numeric_messages {
        ...
        match result {
            Ok(msg) => {
                println!("Parsed malformed numeric '{}' ({}): {}", malformed_msg, description, msg);
            ...
            Err(parse_err) => {
                let error_msg = format!("{}", parse_err);
                println!("Numeric parse error '{}' ({}): {}", malformed_msg, description, error_msg);
                assert!(!error_msg.is_empty(), "Should provide error description");
            }
        }
    }
...
fn test_error_display_formatting() {
    ...
    match result {
        Err(err) => {
            ...
            assert!(!display_format.is_empty(), "Display format should not be empty");
            assert!(!debug_format.is_empty(), "Debug format should not be empty");
            
            // Display format should be user-friendly
            assert!(!display_format.contains("ParseError"), "Display format should not expose internal types");
...
fn test_error_recovery_scenarios() {
    ...
    let recovery_test_messages = vec![
        ("PING :server\r\nPRIVMSG #channel :Hello", "valid message after error"),
        ("INVALID\r\nPING :server", "valid PING after invalid command"),
        ("@bad-tag\r\n@time=2023-01-01T00:00:00Z PING :server", "good tags after bad tags"),
    ];
    ...
    for (test_input, description) in recovery_test_messages {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            match result {
                Ok(msg) => {
                    println!("Line {} of '{}' ({}): parsed as {}", i, test_input, description, msg);
                ...
                Err(err) => {
                    println!("Line {} of '{}' ({}) failed: {}", i, test_input, description, err);
                }
            }
        }
    }
