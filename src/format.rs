...
/// - NUL (0x00) - always illegal, string terminator
/// - BEL (0x07) - always illegal in message content
/// - Any other control character that is NOT CR, LF, or a recognized format code
///
...
///
/// // Always illegal
/// assert!(is_illegal_control_char('\x00')); // NUL
...
pub fn is_illegal_control_char(ch: char) -> bool {
    // NUL and BEL are always illegal
    if ch == '\0' || ch == '\x07' {
        return true;
    }
    // Other control chars are illegal unless they are CR, LF, or a format code
    ch.is_control() && ch != '\r' && ch != '\n' && !is_irc_format_code(ch)
}
